...
gnssMeas.AngleZ   = zeros(N,M)+NaN;
gnssMeas.AngleX   = zeros(N,M)+NaN;
gnssMeas.AngleY   = zeros(N,M)+NaN;
...
AngleZ      = gnssRaw.AngleZ;
AngleX      = gnssRaw.AngleX;
AngleY      = gnssRaw.AngleY;
...
for i=1:N %i is index into gnssMeas.FctSeconds and matrix rows
    ...
    for j=1:length(J) %J(j) is index into gnssRaw.*
        ...
        %Ignore L5 Data
        %{s
        if isfield(gnssRaw, 'CarrierFrequencyHz') &&...
                gnssRaw.CarrierFrequencyHz(J(j)) < 1.57e9
            continue;
        end
        %}
        ...
        gnssMeas.AngleZ(i,k)     = AngleZ(J(j));
        gnssMeas.AngleX(i,k)     = AngleX(J(j));
        gnssMeas.AngleY(i,k)     = AngleY(J(j));
        gnssMeas.AzDeg(i,k)      = 1;
        gnssMeas.ElDeg(i,k)      = 1;
        
        if gnssMeas.AngleX(i,k) < 0
            gnssMeas.AngleZ(i,k) = mod(gnssMeas.AngleZ(i,k)+180,360);
        end
    end
