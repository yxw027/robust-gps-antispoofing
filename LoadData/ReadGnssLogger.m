...
for i=1:size(dataFilter,1)
    j_freq=find(strcmp(header,dataFilter{i,1}));%j = index into header
    %we should always be a value of j, because checkDataFilter checks for this:
    assert(any(j_freq),'dataFilter{i} = %s not found in header\n',dataFilter{i,1})
    ...
    ts = sprintf('%s = C{%d};',header{j_freq},j_freq);
    eval(ts);
...
%deal with L5 frequency measurements
%{
M = length(gnssRaw.Svid);
strSvid = cell(M,1);

for k = 1:M
    if gnssRaw.CarrierFrequencyHz(k) > 1.57542e9
        strSvid{k} = sprintf('%d',gnssRaw.Svid(k));
    else
        strSvid{k} = sprintf('%d-L5',gnssRaw.Svid(k));
    end
end
gnssRaw.Svid = strSvid;
%}
