function [prec, tpr, fpr, thresh,p] = prec_rec(score, target, varargin)
...
%                         lines.
%      - 'pos_dir'      : 'large' or 'small', specify if the num is larger
%                         or smaller than the threshould, the sample will
%                         be classified as positive, default as 'large'. 
...
while (i <= optargin)
    ...
    elseif (strcmp(varargin{i}, 'pos_dir'))
        if (i >= optargin)
            error('argument required for %s', varargin{i});
        else
            pos_dir = varargin{i+1};
            i = i + 2;
        end    
...
for i = 1:length(thresh)
    if strcmp(pos_dir,'large') == 1
        idx     = (score >= thresh(i)); % change the symbol according to the datasets
    else
        idx     = (score <= thresh(i));
    end 
...
if (plot_pr || plot_roc)
    ...
    if (~hold_fig)
        ...
        if (plot_pr)
            ...
            p=plot([tpr;0], [prec;1], style); % add pseudo point to complete curve
            
            xlabel('recall','FontSize',30);
            ylabel('precision','FontSize',30);
            title('precision-recall graph','FontSize',30);
        end
        if (plot_roc)
            if (plot_pr)
                subplot(1,2,2);
            ...
            hold on;
            hold all;
%             p=plot([fpr], [tpr], style); 
            p=plot([0;fpr], [0;tpr], style,'MarkerIndices',1:70:length(tpr)+1, 'MarkerSize',20); % add pseudo point to complete curve
        ...
        if (plot_roc)
            ...
            if strcmp(pos_dir,'large') == 1
                p=plot([0;fpr], [0;tpr], style,'MarkerIndices',1:80:length(tpr)+1,'MarkerSize',20);
            else
                p=plot([fpr;0], [tpr;0], style,'MarkerIndices',1:80:length(tpr)+1,'MarkerSize',20);
            end
        end
    end
end
